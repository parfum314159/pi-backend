...
app.get("/book", async (req, res) => {
  try {
    ...
    if (!doc.data().approved) return res.status(404).json({ success: false, error: "Book not found" });
...
app.post("/save-book", async (req, res) => {
  try {
    ...
    if (
      typeof title !== "string" || typeof owner !== "string" ||
      typeof ownerUid !== "string" || typeof cover !== "string" || typeof pdf !== "string"
    ) {
      return res.status(400).json({ error: "Invalid data" });
    }

    if (!cover.includes("cloudinary.com") || !pdf.includes("cloudinary.com")) {
    ...
    await db.doc("stats/platform").set(
      { totalBooks: admin.firestore.FieldValue.increment(1) },
      { merge: true }
    );
...
app.post("/complete-payment", async (req, res) => {
  ...
  try {
    ...
    await db.runTransaction(async (t) => {
      ...
      t.set(db.doc("stats/platform"), {
        platformProfit: admin.firestore.FieldValue.increment(platformProfit)
      }, { merge: true });
...
/*
 * ================================================================
 * ================= PAYOUT - Pi SDK A2U (App to User) ============
 * ================================================================
 *
 * بدلاً من Stellar SDK، نستخدم Pi Platform API مباشرةً
 * عبر نقطة نهاية /v2/payments (A2U Payment)
 *
 * الخطوات:
 * 1. الواجهة الأمامية تستدعي /init-payout لتحضير الدفعة وتأمينها
 * 2. السيرفر يبدأ دفعة A2U عبر Pi API ويُعيد paymentId
 * 3. الواجهة تستدعي Pi.createPayment() مع paymentId (للتوقيع من جانب المستخدم)
 * 4. Pi يستدعي /approve-payout (server callback) لإكمال العملية
 * 5. السيرفر يُكمل الدفعة ويُصفّر الأرباح
 * ================================================================
 */

/* ================= INIT PAYOUT (A2U) ================= */
app.post("/init-payout", async (req, res) => {
  try {
    const { userUid, accessToken } = req.body;
    ...
    const amount = parseFloat(totalEarnings.toFixed(2));

    // إنشاء دفعة A2U عبر Pi Platform API
    // Pi SDK من جانب السيرفر لإنشاء الدفعة
    const paymentResponse = await fetch(`${PI_API_URL}/payments`, {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        payment: {
          ...
          uid: userUid   // uid المستخدم الذي سيستلم الدفعة
        }
      })
    });
    ...
    if (!paymentResponse.ok) {
      ...
      await payoutLockRef.delete();
      return res.status(500).json({ error: "Failed to create Pi payment: " + errText });
    }
    ...
    if (!paymentId) {
      await payoutLockRef.delete();
      return res.status(500).json({ error: "No payment ID returned from Pi" });
    }

    // حفظ الدفعة المعلقة في Firestore
    ...
    console.error("Init payout error:", err);
    try {
...
/* ================= APPROVE PAYOUT (Pi callback A2U) ================= */
app.post("/approve-payout", async (req, res) => {
  try {
    const { paymentId } = req.body;
    if (!paymentId) return res.status(400).json({ error: "Missing paymentId" });

    // التحقق من الدفعة في Pi
    const paymentInfo = await fetch(`${PI_API_URL}/payments/${paymentId}`, {
      method: "GET",
      headers: { Authorization: `Key ${PI_API_KEY}` }
    });
...
/* ================= COMPLETE PAYOUT (Pi callback A2U) ================= */
app.post("/complete-payout", async (req, res) => {
  try {
    ...
    if (!userUid) throw new Error("Missing metadata");
    ...
    await db.collection("pendingPayouts").doc(paymentId).delete();
    try {
      await db.collection("payoutLocks").doc(userUid).delete();
    } catch {}
